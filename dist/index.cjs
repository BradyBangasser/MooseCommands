...
__export(src_exports, {
  CommandBase: () => commandBase_default,
  MessActions: () => messactions_extentions_default,
  MooseCache: () => MooseCache2,
  MooseChannel: () => channel_extention_default,
  MooseError: () => MooseError,
  MooseGuild: () => guild_extentions_default,
  MooseMember: () => MooseMember,
  MooseRole: () => MooseRole2,
  MooseUser: () => MooseUser,
  default: () => src_default
});
...
__export(functions_exports, {
  ...
  wait: () => import_promises.setTimeout
});
...
function getFiles(dirpath = process.cwd()) {
  const files = import_fs.default.readdirSync(dirpath, { withFileTypes: true });
  let funkyFiles = [];
  files.forEach((file) => {
    if (file.isDirectory())
      funkyFiles = [...funkyFiles, ...getFiles(import_path.default.join(String(dirpath), file.name))];
...
var DataBaseManager = class {
  ...
  async setup() {
    try {
      this._connection = await import_mongoose.default.connect(this.mongoURI, this.options);
  ...
  async _getSchemas(schemaPath = import_path2.default.join(__dirname, "schemas")) {
    ...
    for (const [filePath, fileName] of files) {
      ...
      if (content instanceof import_mongoose.default.Schema) {
        this._models[modelName] = import_mongoose.default.model(modelName, content);
      }
    }
  }
  get connection() {
  ...
  _registerModel(modelName, model) {
    this._models[modelName] = import_mongoose.default.model(modelName, model);
  }
  getModel(modelName) {
...
var DisCrackChalk = class extends import_chalk.Chalk {
  constructor(level) {
    super({ level });
  }
};
...
var MooseLogger = class {
  ...
  log(text, priority = "none", style = this._options) {
    let newValue = text;
    if (style.box) {
      newValue = import_figlet.default.textSync(newValue, style);
    }
...
var MooseClient = class {
  constructor(token, options) {
    ...
    this._client = new import_discord.Client(options);
...
var MessActions2 = class {
  ...
  constructor(message, client) {
    ...
    this.stickers = message.stickers ?? new import_discord2.Collection();
    ...
    this.attachments = message.attachments ?? new import_discord2.Collection();
...
var SlashCommandOptionClassesEnum = {
  "int": import_discord3.SlashCommandIntegerOption,
  "number": import_discord3.SlashCommandNumberOption,
  "float": import_discord3.SlashCommandNumberOption,
  "double": import_discord3.SlashCommandNumberOption,
  "integer": import_discord3.SlashCommandIntegerOption,
  "string": import_discord3.SlashCommandStringOption,
  "user": import_discord3.SlashCommandUserOption,
  "boolean": import_discord3.SlashCommandBooleanOption,
  "subcommand": import_discord3.SlashCommandSubcommandBuilder,
  "subcommandgroup": import_discord3.SlashCommandSubcommandGroupBuilder,
  "channel": import_discord3.SlashCommandChannelOption,
  "mentionable": import_discord3.SlashCommandMentionableOption,
  "role": import_discord3.SlashCommandRoleOption,
  "attachment": import_discord3.SlashCommandAttachmentOption
};
...
var CommandBase = class {
  ...
  _createFormatClasses(format, builder = new import_discord4.SlashCommandBuilder()) {
...
var MooseChannel = class {
  ...
  constructor(client, channel) {
    ...
    if (!(channel instanceof import_discord5.DMChannel || channel instanceof import_discord5.PartialGroupDMChannel)) {
